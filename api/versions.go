					...
					for name, allLabels := range contStat.CustomMetrics {
						...
						for _, metric := range allLabels {
							...
							if !metric.Timestamp.IsZero() {
								...
								if metric.Label != "" {
									tempLabels := metrics[name]
									if tempLabels != nil {
										values := tempLabels[metric.Label]
										values = append(values, metVal)
										tempLabels[metric.Label] = values
										metrics[name] = tempLabels
									} else {
									...
									if tempLabels != nil {
										...
										metricLabels[""] = []info.MetricValBasic{metVal}
										metrics[name] = metricLabels
									}
						...
						//metrics[name] = append(metrics[name], metricLabels)
					}
				}
			}
